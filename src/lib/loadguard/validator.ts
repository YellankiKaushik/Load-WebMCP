...
export function volumeCm3(p: Package) {
  return [p.lengthCm, p.widthCm, p.heightCm].every(positiveFinite)
    ? p.lengthCm * p.widthCm * p.heightCm
    : 0;
}

export function truckVolumeCm3(t: Truck) {
  return [t.lengthCm, t.widthCm, t.heightCm].every(positiveFinite)
...
export function validatePlan(
  ...
  for (const item of placements) {
    ...
    if (!pkg) {
      violations.push({
        ...
        severity: "hard",
        boxCodes: [item.boxCode],
        message: `${item.boxCode} is not part of this truck/session manifest.`,
      });
      continue;
    }
    if (seenPlacements.has(item.boxId)) {
      violations.push({
        code: "DUPLICATE_PLACEMENT",
        severity: "hard",
        boxCodes: [pkg.code],
        message: `${pkg.code} has more than one proposed placement.`,
      });
      continue;
    }
    seenPlacements.add(item.boxId);

    if (
      !validCoordinate(item.position.x) ||
      !validCoordinate(item.position.y) ||
      !validCoordinate(item.position.z)
    ) {
      violations.push({
        code: "INVALID_POSITION",
        severity: "hard",
        boxCodes: [pkg.code],
        message: `${pkg.code} has a malformed or negative position.`,
      });
      continue;
    }
  ...
  if (truckNums.some((n) => !positiveFinite(n))) {
    violations.push({
      code: "INVALID_PACKAGE",
      severity: "hard",
      boxCodes: [truck.code],
      message: `${truck.code} has invalid dimensions or weight capacity.`,
    });
  }
  ...
  for (const { pkg } of placed) {
    ...
    if (nums.some((n) => !positiveFinite(n))) {
      violations.push({
        code: "INVALID_PACKAGE",
        severity: "hard",
        boxCodes: [pkg.code],
