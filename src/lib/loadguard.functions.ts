...
export const fetchLoadState = createServerFn({ method: "POST" })
    .inputValidator((data) =>
        z
            .object({ actor: z.enum(["agent", "human", "system"]).default("human") })
            .parse(data ?? {}),
    )
    .handler(async ({ data }) => {
...
export const fetchPackageConstraints = createServerFn({ method: "POST" })
    .inputValidator((data) => z.object({ codes: z.array(z.string()).optional() }).parse(data ?? {}))
    .handler(async ({ data }) => {
...
export const planLoad = createServerFn({ method: "POST" })
    .inputValidator((data) =>
        z.object({ includeCodes: z.array(z.string()).optional() }).parse(data ?? {}),
    )
    .handler(async ({ data }) => {
...
export const validateLoadPlan = createServerFn({ method: "POST" })
    .inputValidator((data) =>
        z.object({ planId: z.string().uuid().optional() }).parse(data ?? {}),
    )
    .handler(async ({ data }) => {
