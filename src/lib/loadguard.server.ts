...
async function planSummary(planId: string): Promise<PlanSummary | null> {
...
export async function createCandidatePlan(sessionKey: string, includeCodes?: string[]) {
    ...
    return {
        plan: await planSummary(plan.id),
        stateRevision: truck.stateRevision,
...
export async function validateLoad(sessionKey: string, planId?: string) {
    ...
    if (!planId) {
        ...
        await log(sessionKey, {
            ...
            summary: `Active load at revision ${truck.stateRevision}: ${validation.valid ? "no hard violations" : `${validation.violations.length} violation(s)`
                }.`,
        });
        return {
            ok: true as const,
    ...
    const summary = await planSummary(planId);
...
export async function stagePlan(sessionKey: string, planId: string) {
    ...
    if (
        summary.sourceStateRevision !== null &&
        summary.sourceStateRevision !== truck.stateRevision
    ) {
        await log(sessionKey, {
            actor: "agent",
            eventType: "stage",
    ...
    const { data: hash } = await supabaseAdmin.rpc("canonical_plan_hash", { p_plan_id: planId });
    ...
    await supabaseAdmin
        ...
        .update({
            ...
            plan_hash: hash as unknown as string,
    ...
    return { ok: true as const, plan: await planSummary(planId) };
}

export async function approvePlan(sessionKey: string, planId: string) {
    const before = await planSummary(planId);
    const { data, error } = await supabaseAdmin.rpc("approve_load_plan", {
        p_session_key: sessionKey,
        p_plan_id: planId,
    });
    const result = (data ?? { ok: false, code: error?.message ?? "RPC_FAILED" }) as {
        ok: boolean;
        code?: string;
    };
    ...
    return { ...result, plan: await planSummary(planId) };
}

export async function rejectPlan(sessionKey: string, planId: string) {
    const before = await planSummary(planId);
    const { data, error } = await supabaseAdmin.rpc("reject_load_plan", {
        p_session_key: sessionKey,
        p_plan_id: planId,
    });
    const result = (data ?? { ok: false, code: error?.message ?? "RPC_FAILED" }) as {
...
export async function commitPlan(sessionKey: string, planId: string) {
    const before = await planSummary(planId);
    const { data, error } = await supabaseAdmin.rpc("commit_load_plan", {
        p_session_key: sessionKey,
        p_plan_id: planId,
    });
    const result = (data ?? { ok: false, code: error?.message ?? "RPC_FAILED" }) as {
