...
export function StatusBadge({ status }: { status: string }) {
    return (
        <span
            className={`rounded border px-2 py-0.5 font-mono text-[0.65rem] uppercase tracking-[0.15em] ${statusStyles[status] ?? "text-muted-foreground border-border"
                }`}
        >
            {status}
        </span>
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-2">
                <div className="font-mono text-xs text-muted-foreground">
                    {plan.planId.slice(0, 8)} · {plan.algorithmVersion}
                </div>
            ...
            <div className="grid grid-cols-2 gap-2 font-mono text-[0.7rem] text-muted-foreground">
                <div>
                    Utilization <span className="text-foreground">{plan.utilizationPct ?? "—"}%</span>
                </div>
                <div>
...
export function McpStatus({
    ...
    return (
        <div className="space-y-3">
            ...
            <p className="text-[0.7rem] text-muted-foreground">
                Approval is deliberately not a tool. Commit accepts only a proposal id and is enforced in the
                database.
            </p>
        </div>
    );
