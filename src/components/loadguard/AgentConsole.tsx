    ...
    const call = useMutation({
        mutationFn: async (tool: "create" | "validate" | "stage" | "commit") => {
            ...
            if (tool === "validate") {
                ...
                return `validate_load_plan → ${r.validation.valid ? "valid" : `${r.validation.violations.length} violation(s)`
                    }`;
            }
            if (tool === "stage") {
            ...
            <div className="grid grid-cols-2 gap-2">
                {buttons.map((b) => (
                    <button
                        ...
                        className="rounded-md border border-border bg-secondary/50 px-2 py-2 font-mono text-[0.65rem] text-foreground transition-colors hover:border-primary/60 hover:text-primary disabled:opacity-50"
                    >
                        {b.label}
                    </button>
                ))}
                ...
                {log.length === 0 ? (
                    ...
                    <ul className="space-y-1 font-mono text-[0.65rem]">
                        {log.map((entry, i) => (
                            <li
                                key={i}
                                className={entry.tone === "ok" ? "text-success" : "text-destructive"}
                            >
                                {entry.tone === "ok" ? "» " : "✗ "}
                                {entry.text}
                            </li>
